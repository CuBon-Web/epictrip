...
class PageController extends Controller
{
    ...
    public function bookATrrip(){
        return view('bookatrip');
    }
    ...
    public function quickview(Request $request){
        $pro = Founder::where('id',$request->id)->first();
            ...
            try {
                ...
                foreach ($notifyEmails as $to) {
                    if (filter_var($to, FILTER_VALIDATE_EMAIL)) {
                        Mail::to($to)->send(new ContactMessageMail($data, false));
                    }
                }
                $customerEmail = trim((string) $request->email);
                if ($customerEmail !== '' && filter_var($customerEmail, FILTER_VALIDATE_EMAIL)) {
                    Mail::to($customerEmail)->send(new ContactMessageMail($data, true));
                }
            } catch (\Throwable $e) {
                Log::error('Contact form email failed: ' . $e->getMessage(), [
                    'mess_contact_id' => $data->id,
                    'exception' => $e,
                ]);
            }
    ...
    public function postBookService(Request $request)
    {
        ...
        $service = Services::where('id', $validated['service_id'])->where('status', 1)->first();
        if (! $service) {
            return back()->withInput()->with('error', 'Dịch vụ không tồn tại hoặc đã ngừng hiển thị.');
        }

        $booking = new ServiceBooking();
        $booking->service_id = (int) $validated['service_id'];
        ...
        $booking->total = isset($validated['total']) ? (float) $validated['total'] : 0;
        $booking->save();

        $serviceTitle = languageName($service->name);
        $setting = Setting::first();
        ...
        //     foreach ($notifyEmails as $to) {
        //         if (filter_var($to, FILTER_VALIDATE_EMAIL)) {
        //             Mail::to($to)->send(new ServiceBookingMail($booking, $serviceTitle, false));
        //         }
        //     }
        //     $customerEmail = trim((string) ($validated['email'] ?? ''));
        //     if ($customerEmail !== '' && filter_var($customerEmail, FILTER_VALIDATE_EMAIL)) {
        //         Mail::to($customerEmail)->send(new ServiceBookingMail($booking, $serviceTitle, true));
        //     }
        // } catch (\Throwable $e) {
        //     Log::error('Service booking email failed: ' . $e->getMessage(), [
        //         'service_booking_id' => $booking->id,
        //         'exception' => $e,
        //     ]);
        // }

        return view('servicebooksuccess', compact('booking'));
    }
    ...
    public function feedback() {
        $data['data'] = ReviewCus::where('status',1)->get();
        return view('feedback',$data);
    }
    public function gallery()
    {
        $data['list'] = AlbumAffter::where('status',1)->paginate(12);
