...
if(!function_exists('languageName')){
    function languageName($arrName){
        ...
        if (is_numeric($arrName)) {
            return $arrName;
        }

        if (is_array($arrName) || is_object($arrName)) {
            $arr = $arrName;
        } else {
...
if (!function_exists('getYoutubeId')) {
    function getYoutubeId($url)
    {
        ...
        $pattern = '~(?:youtu\.be/|youtube(?:-nocookie)?\.com/(?:watch\?v=|embed/|shorts/|v/|live/))([A-Za-z0-9_-]{11})~i';
...
if (!function_exists('detectMediaType')) {
    function detectMediaType($url)
    {
        $url = trim((string) $url);
        if ($url === '') {
            return ['type' => 'none', 'url' => '', 'youtubeId' => null, 'mime' => null];
        }
        $ytId = getYoutubeId($url);
        if ($ytId) {
            return ['type' => 'youtube', 'url' => $url, 'youtubeId' => $ytId, 'mime' => null];
        }
        ...
        if (in_array($ext, $videoExts, true)) {
            ...
            return ['type' => 'video', 'url' => $url, 'youtubeId' => null, 'mime' => $mime];
        }
        return ['type' => 'image', 'url' => $url, 'youtubeId' => null, 'mime' => null];
    }
}
