...
use App\Services\SeoPingService;
use DB;
class Product extends Model
{
    ...
    private function defaultLangContent($value)
    {
        if (is_string($value)) {
            $decoded = json_decode($value, true);
            if (json_last_error() === JSON_ERROR_NONE && is_array($decoded)) {
                return $this->defaultLangContent($decoded);
            }
        }

        if (is_array($value)) {
            $first = $value[0] ?? [];
            return is_array($first) ? ($first['content'] ?? '') : '';
        }
        ...
        try {
            if ($id) {
                ...
                if($query){
                    ...
                    $query->slug = to_slug($this->defaultLangContent($request->name));
                    $query->price = json_encode($request->price);
                    $query->price_chil = json_encode($request->price_chil);
                    ...
                    $query->tag_cate = $request->tag_cate ?? 0;
