<template>
  <div>
    <h3 class="page-title">Đơn hàng dịch vụ</h3>
    <div class="row">
        ...
        <div class="card">
          <div class="card-body">
            <div class="row" v-if="items.length == 0">
              ...
              <div class="col-md-4">
                <div class="mb-0 flex-grow">
                  <h5 class="mr-2 mb-2">Quản lý đơn đặt dịch vụ</h5>
            ...
            <vs-input
              v-if="items.length > 0"
              icon="search"
              placeholder="Tìm theo tên khách, SĐT, email, tên dịch vụ…"
              v-model="keyword"
              ...
              <template slot-scope="{data}">
                <vs-tr :key="'row'+indextr" v-for="(tr, indextr) in data">
                  ...
                  <vs-td :data="tr.service_name">
                    <strong>{{ tr.service_name || ('#' + tr.service_id) }}</strong>
                    <div v-if="tr.service_public_url" class="text-muted small" style="margin-top:4px;">
                      <a :href="tr.service_public_url" target="_blank" rel="noopener">Xem trang dịch vụ</a>
                    </div>
                  </vs-td>
                  ...
                  <vs-td :data="tr.id">
                    <router-link :to="{ name: 'bill_service_detail', params: { id: tr.id } }">
...
export default {
  ...
  methods: {
    ...mapActions(["listServiceBooking", "deleteServiceBooking", "loadings"]),
    formatDate(value) {
      if (!value) return "";
    ...
    fetchList() {
      ...
      this.listServiceBooking({ keyword: this.keyword })
        .then((res) => {
