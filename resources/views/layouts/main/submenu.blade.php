               ...
               <div class="subcate gd-menu">
                  <div class="nd-cate-list ">
                     ...
                     @foreach ($item->tagCate as $tagcate)
                        @if ($tagcate->status == 1)
                        <aside>
                           <a class="nd-categories-main-sub" href="" title="{{ languageName($tagcate->name) }}">{{ languageName($tagcate->name) }}</a>
                           @if (count($tagcate->tags) > 0)
                           <ul class="submenu__list">
                              @foreach ($tagcate->tags as $tag)
                              <li class="submenu__col">
                                 <span class="submenu__item submenu__item--main">
                                     <a class="link" href="" title="{{ languageName($tag->name) }}">{{ languageName($tag->name) }}</a>
                                 </span>
                             </li>
                              @endforeach
                          </ul>
