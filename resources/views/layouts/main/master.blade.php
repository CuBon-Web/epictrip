...
<!DOCTYPE html>
<html lang="{{ session('localelang', config('app.locale')) }}" data-x="html" data-x-toggle="html-overflow-hidden">
   <head>
      @php
      ...
      <meta http-equiv="Content-Language" content="{{ session('localelang', config('app.locale')) }}" />
      <link rel="canonical" href="{{ $seoCanonical }}" />
      <link rel="alternate" href="{{ $seoCanonical }}" hreflang="en-US" />
      <link rel="alternate" href="{{ $seoCanonical }}" hreflang="es-ES" />
      <link rel="alternate" href="{{ $seoCanonical }}" hreflang="x-default" />

      <meta property="og:type" content="website" />
      <meta property="og:locale" content="{{ str_replace('-', '_', session('localelang', config('app.locale'))) }}" />
   ...
   <body>
      ...
      <div class="page-wraper">
         ...
         @php
             ...
             $hotlineTel = preg_replace('/\s+/', '', $hotlineNumber);
            $phoneDigits = preg_replace('/\D+/', '', $hotlineNumber);
            $whatsappPhone = $phoneDigits;
            if ($whatsappPhone !== '' && strlen($whatsappPhone) >= 9 && substr($whatsappPhone, 0, 1) === '0') {
                $whatsappPhone = '84' . substr($whatsappPhone, 1);
            } elseif ($whatsappPhone !== '' && strlen($whatsappPhone) >= 9 && substr($whatsappPhone, 0, 2) !== '84') {
                $whatsappPhone = '84' . $whatsappPhone;
            }
            $whatsappHref = $whatsappPhone !== '' ? 'https://wa.me/' . $whatsappPhone : route('lienHe');
             $fbRaw = trim((string) ($setting->facebook ?? ''));
             $messengerHref = route('lienHe');
             if ($fbRaw !== '') {
                 $fbLower = strtolower($fbRaw);
                 $looksSocial = (strpos($fbLower, 'm.me/') !== false)
                     || (strpos($fbLower, 'messenger.com') !== false)
                     || (strpos($fbLower, 'facebook.com/') !== false)
                     || (strpos($fbLower, 'fb.com/') !== false);
                 if ($looksSocial) {
                     if (stripos($fbRaw, 'm.me/') !== false || stripos($fbRaw, 'messenger.com') !== false) {
                         $messengerHref = $fbRaw;
                     } elseif (preg_match('#(?:https?://)?(?:www\.)?facebook\.com/([^/?#]+)#i', $fbRaw, $m)) {
                         $page = $m[1];
                         if (strtolower($page) !== 'profile.php' && strtolower($page) !== 'share') {
                             $messengerHref = 'https://m.me/' . $page;
                         } else {
                             $messengerHref = $fbRaw;
                         }
                     } elseif (preg_match('#(?:https?://)?(?:www\.)?fb\.com/([^/?#]+)#i', $fbRaw, $m)) {
                         $messengerHref = 'https://m.me/' . $m[1];
                     } else {
                         $messengerHref = $fbRaw;
                     }
                 }
         ...
         <div class="smart-contact" style="position:fixed; right:20px; bottom:20px; z-index:1050;">
             ...
             <div class="smart-actions" style="display:none;">
                 <a href="tel:{{ $hotlineTel }}"
                     class="smart-btn" aria-label="Gọi hotline"
                ...
                <a href="{{ $whatsappHref }}" target="_blank" rel="noopener"
                    class="smart-btn" aria-label="Chat WhatsApp"
                 ...
                 <a href="{{ $messengerHref }}" target="_blank" rel="noopener"
                     class="smart-btn" aria-label="Facebook Messenger"
                     style="display:inline-flex; align-items:center; justify-content:center; width:48px; height:48px; border-radius:50%; background:linear-gradient(145deg,#00B2FF,#006AFF); color:#fff; box-shadow:0 6px 16px rgba(0,0,0,0.15);">
                     <i class="bi bi-messenger" style="font-size:22px;"></i>
                 </a>
             </div>
