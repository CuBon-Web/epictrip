...
(function ($) {
	...
	function trv_hp5_slider(){
		var swiper = new Swiper(".trv_hp5-slider", {
			...
			navigation: {
				nextEl: ".trv_hp5-slider .swiper-button-next",
				prevEl: ".trv_hp5-slider .swiper-button-prev",
			},
	...
	function trv_tour_cat_carousal(){
		...
		const updateFeatureContent = () => {
			const activeSlide = document.querySelector(".trv-cat-sld.swiper-slide-active");
			if (!activeSlide || !featureTitle || !featureDescription) {
				return;
			}
			featureTitle.textContent = activeSlide.getAttribute("data-title") || "";
			featureDescription.textContent = activeSlide.getAttribute("data-description") || "";
		};

		if (!document.querySelector(".trv-tr-cat-carousal")) {
			return;
		}
		...
		on: {
			...
			if (featureDescription) featureDescription.classList.add("animate-description");
			...
			setTimeout(() => {
				...
				if (featureDescription) featureDescription.classList.remove("animate-away");
